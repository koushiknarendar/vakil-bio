...
export async function GET(request: NextRequest) {
  ...
  const storedState = request.cookies.get('oauth_google_state')?.value
  ...
  try {
    const tokenRes = await fetch('https://oauth2.googleapis.com/token', {
      ...
      body: new URLSearchParams({
        ...
        redirect_uri: `${BASE()}/api/calendar/google/callback`,
    ...
    return NextResponse.redirect(`${BASE()}/dashboard/calendar?error=google_auth_failed`)
  }
  ...
  const response = NextResponse.redirect(`${BASE()}/dashboard/calendar?connected=google`)
